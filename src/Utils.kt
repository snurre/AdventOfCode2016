...
fun String.md5(): String = md.digest(this.toByteArray()).toHex()
...
class Path<T>(
    ...
    tailrec fun getShortestPath(
        ways: List<AStarElement> = listOf(aStarElement(listOf(start), 0)),
        walked: Set<Point> = emptySet()
    ): List<Point>? {
    ...
    private fun aStarElement(
        way: List<Point>,
        realCost: Int
    ): AStarElement = AStarElement(way, realCost, goal.heuristicCost(way.last()))

    private fun neighbors(element: AStarElement): List<AStarElement> =
        neighborsAndCost(element.wayEnd).map { (point, cost) ->
            aStarElement(
                element.way + point,
                element.realCost + cost
            )
        }

    private fun neighborsAndCost(point: Point): List<Pair<Point, Int>> = listOf(
        Point(point.x, point.y - 1) to 2,
        Point(point.x - 1, point.y) to 2,
        Point(point.x + 1, point.y) to 2,
        Point(point.x, point.y + 1) to 2
    ).filter { isValid(it.first) }

    private fun isValid(point: Point): Boolean =
        point.x in 0 until map[0].size - 1 && point.y in 0 until map.size && map[point.y][point.x] == ' '
}

data class AStarElement(
    val way: List<Point>,
    val realCost: Int,
    private val heuristicCost: Int
) : Comparable<AStarElement> {
